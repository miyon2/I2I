...
#define MQTT_HOSTNAME "192.168.0.166"
...
int ioctl_sensor_refresh(){
	unsigned long value;
	ioctl(dev, REFRESH_DATA, &value);
	return 0;
}
...
int mosq_pub_temp(){
    ...
    printf("[tmp]my_dht is : %d.%d\n", my_dht->dht3, my_dht->dht4);
...
int mosq_pub_hum(){
    ...
    ioctl(dev, GET_HUMIDITY, my_dht);
    printf("[hum]my_dht is : %d.%d\n", my_dht->dht1, my_dht->dht2);
